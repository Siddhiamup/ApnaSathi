...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $requireAmount = mysqli_real_escape_string($conn, $_POST['require']);

    // File upload directory
    $upload_dir = 'uploads/Supporter';
    ...
    if (move_uploaded_file($_FILES['photo']['tmp_name'], $photo_path) && move_uploaded_file($_FILES['idProof']['tmp_name'], $idProof_path)) {
        ...
        $sql = "INSERT INTO financial_beneficiary(user_id, beneficiary_name, beneficiary_mobile, beneficiary_birthdate, age, beneficiary_photo,beneficiary_id_proof,purpose,requireAmount,beneficiary_email)
                VALUES (' $ $beneficiary_id ', '$beneficiary_name', '$beneficiary_mobile', '$beneficiary_birthdate', '$age', '$photo_path','$idProof_path','$purpose','$requireAmount','$beneficiary_email')";
